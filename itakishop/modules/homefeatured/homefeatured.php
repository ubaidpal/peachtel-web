...
class HomeFeatured extends Module
{
	...
	public function hookDisplayHome($params)
	{
		...
		$products = $category->getProducts((int)Context::getContext()->language->id, $page, ($nb ? $nb : 10));
		$productsCount = $category->getProducts((int)Context::getContext()->language->id, 1, 9999);
		$nbProducts = $products;
		$count = (count($productsCount) / 9);
		
		$stop = 3;
		if($page >= $stop && $page < $count) {
			$stop = $page + 1;
		} else if($page == $count) {
			$stop = $page;
		} else {
		...
		$this->smarty->assign(array(
			...
			'start' => 1,
			'stop' => $stop,
			'pages_nb' => ($count > intval($count)) ? intval($count) + 1 : $count,
			'products_per_page' => $nb,
