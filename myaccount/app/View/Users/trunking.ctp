        ...
        <div class="trunking_main_holder">
            ...
            <div class="trunking_content">
                ...
                <div class="box">
                    ...
                    <div class="label_capt table_title"> Trunking Status:  
                        <label class="activeInactiveImg"><?php echo ($clientTrunkData['Customer']['active'] == "t") ? "Active" : "Inactive"; ?></label>
                    </div>
                    ...
                    <div class="label_capt table_title table_capt" style="border-bottom:none;"><h4>Billing Group's List:</h4> [<?php echo $this->Html->link('Add', 'javascript:void(0)', array('id' => 'addBillingGroupBtn')); ?>]</div>
                    ...
                    <!-- DID Routes table -->
                    <div class="label_capt table_title table_capt"><h4>DID Routes:</h4>&nbsp; <label><a href="javascript:void(0);" id="orderNewNumbers">Add new DID(s)</a></label></div>
                    <div style="max-height: 500px; overflow-y: auto;">
                    <table cellspacing="0" id="subs_table" style="margin:0;">
                        <tr>
                            <th>Name</th>
                            <th width="250px">Prefix</th>
                            <th width="100px">Active</th>
                            <th width="250px">Cost Destination</th>
                        </tr>
                        <?php if(!empty($routes)) : ?>
                        <?php foreach($routes as $route) : ?>
                        <tr>
                            <td><?php echo ($route['OriginationRoute']['descr']) ? $route['OriginationRoute']['descr'] : "Unnamed"; ?></td>
                            ...
                            <td><?php echo $this->itaki->isActive($route['OriginationRoute']['active']); ?></td>
                    ...
                    <!-- Cdr's table -->
                    <div class="label_capt table_title" style="margin-top:20px;"><h4>Cdr's List:</h4>&nbsp;</div><!-- label_capt table_title -->
...
<div id="editCostDestDialog" style="display: none;">
    <?php 
        echo $this->Form->create('user', array('action' => 'updateCostDest'));  
...
$(function() {
    ...
    $('#editCostDest').live('click', function() {
        ...
        $('#userOrigRouteId').val(cid);
        $('#userPriority').val(data.priority);
        $('#userClassDest').val(data.class_dest);
        $('#userFinal').val(data.final);
    ...
    /** JAVASCRIPT FOR DID NUMBERS */
    $("#orderNewNumbers").live('click', function() {
        var data = {};
        globalAjaxQuery('/myaccount/quotes/configureDid', data);
        $("#DIDform").html(globalData);
    ...
    $("#search_did").live('change', function() {
        $("#search_states").find("option:first-child").attr("selected", "selected");
        switch($(this).val()) {
            case "GEOGRAPHIC" :
            case "TOLLFREE" :
                globalAjaxQuery("/myaccount/quotes/getDIDCountries", {did_type: $(this).val()});
                $("#search_country").html(globalData);
                $("#search_country_holder").fadeIn();
                $("#search_states_holder").fadeOut();
                $("#findNumberBtn").fadeIn();
                break;
            case "NATIONAL" :
                $("#search_country_holder").fadeOut();
                var cid = "191.195.165.31.230.75.145.255.244.232.18.72.215.141.74.73.110.154;c4dc4eeed3c5ee9d2fa81c8ca75bb8ea609503d1";
                globalAjaxQuery("/myaccount/quotes/getDIDStates", {did_type: $(this).val(), country_id: cid});
                $("#search_states").html(globalData);
                $("#search_states_holder").fadeIn();
                break;
            default :
                $("#search_country_holder").fadeOut();
                $("#search_states_holder").fadeOut();
                $("#findNumberBtn").fadeOut();
                break;
    ...
    $("#search_country").live('change', function() {
        $("#search_states").find("option:first-child").attr("selected", "selected");
        if($(this).find("option:selected").attr('has_states') == "true") {
            globalAjaxQuery("/myaccount/quotes/getDIDStates", {country_id: $(this).val(), did_type: $("#search_did").val()});
            $("#search_states").html(globalData);
            $("#search_states_holder").fadeIn();
        } else {
            $("#search_states_holder").fadeOut();
    ...
    $("#findNumberBtn").live('click', function() {
        ...
        globalAjaxQuery('/myaccount/quotes/setPhoneNumbers', data);
        $("#found-numbers").html(globalData);
        $('#browser').treeview({
            animated: "fast",
            persist: "location",
            collapsed: true,
    ...
    $(".hitarea").live('click', function() {
        ...
        if($(this).hasClass('collapsable-hitarea') && li.find('ul').html() == '') {
            ...
            ajaxQuery('/myaccount/quotes/getDIDArea', data);
            li.find('ul').css('display', 'block').html(globalData);
        }
    });
    ...
    $(".ui-tabs-nav li:last-child").live('click', function() {
        ...
        var data = {};
        ajaxQuery('/myaccount/quotes/configureSelectedDids', data);
        $("#config-numbers").html(globalData);
    });
...
Quote = {
    ...
    addRemoveNumber: function(method, did, nid) {
        ...
        ajaxQuery('/myaccount/quotes/addRemoveNumber', data);
        
        return parseInt(globalData);
    },
