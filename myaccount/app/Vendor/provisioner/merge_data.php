...
class provisioner {
    ...
    function mergeData($request = null, $brand = null, $product = null, $model= null, $show_array = null, $hide_loops = null, $pp = null) {
        require('includes/json.php');
        define("PROVISIONER_PATH", "../../app/webroot/files/provisioner/");
        $request = ($request) ? $request : '';
        $brand = ($brand) ? $brand : '';
        $product = ($product) ? $product : '';
        $model = ($model) ? $model : '';
        $show_array = ($show_array) ? TRUE : FALSE;
        $hide_loops = ($hide_loops) ? TRUE : FALSE;
        $pp = ($pp) ? TRUE : FALSE;
        ...
        switch ($request) {
            case 'list':
                ...
                if (file_exists(PROVISIONER_PATH . "endpoint/master.json")) {
                    $master = $this->file2json(PROVISIONER_PATH . "endpoint/master.json");
                    foreach ($master['data']['brands'] as $brand_list) {
                        if (file_exists(PROVISIONER_PATH . "endpoint/" . $brand_list['directory'] . "/brand_data.json")) {
                            $brand_data = array();
                            $brand_data = $this->file2json(PROVISIONER_PATH . "endpoint/" . $brand_list['directory'] . "/brand_data.json");
                            ...
                            foreach ($family_list_array as $family_list) {
                                if (file_exists(PROVISIONER_PATH . "endpoint/" . $brand_list['directory'] . "/" . $family_list['directory'] . "/family_data.json")) {
                                    $family_data = array();
                                    $family_data = $this->file2json(PROVISIONER_PATH . "endpoint/" . $brand_list['directory'] . "/" . $family_list['directory'] . "/family_data.json");
            ...
            case 'data':
                if (!empty($brand) && empty($product) && empty($model)) {
                    if (file_exists(PROVISIONER_PATH . "endpoint/" . $brand . "/brand_data.json")) {
                        ...
                        $this->jsondie('No brand_data.json for ' . $brand);
                    }
                } elseif (!empty($brand) && !empty($product) && empty($model)) {
                    if (file_exists(PROVISIONER_PATH . "endpoint/" . $brand . "/" . $product . "/family_data.json")) {
                        $temp = $this->file2json(PROVISIONER_PATH . "endpoint/" . $brand . "/" . $product . "/family_data.json");
                        if ($show_array) {
                            echo "<pre>";
                            print_r($temp['data']);
                        } else {
                            return json_encode($temp['data']);
                        }
                    } else {
                        $this->jsondie('No family_data.json for ' . $product);
                    }
                } elseif (!empty($brand) && !empty($product) && !empty($model)) {
                    ...
                    if (file_exists(PROVISIONER_PATH . "endpoint/" . $brand . "/" . $product . "/family_data.json")) {
                        ...
                        array_unshift($files, PROVISIONER_PATH."endpoint/global_template_data.json");
                        foreach ($files as $files_data) {
                            if (($files_data != 'line_options_22.json') AND (file_exists(PROVISIONER_PATH . "endpoint/" . $brand . "/" . $product . "/" . $files_data))) {
                                $temp_files_data = $this->file2json(PROVISIONER_PATH . "endpoint/" . $brand . "/" . $product . "/" . $files_data);                        
