...
for ( $counter = 1; $counter <= 31; $counter += 1) {
	...
	$query = "SELECT SUM(amountin),SUM(fees),SUM(amountout) FROM tblaccounts INNER JOIN tblclients ON tblclients.id=tblaccounts.userid WHERE date LIKE '$year-$month-$counter%' AND tblclients.currency='$currencyid'";
	$result = mysql_query($query);
	$data = mysql_fetch_array($result);
	$amountin = $data[0];
	$fees = $data[1];
	$amountout = $data[2];
    $query = "SELECT SUM(amountin),SUM(fees),SUM(amountout) FROM tblaccounts WHERE date LIKE '$year-$month-$counter%' AND userid='0' AND currency='$currencyid'";
	$result = mysql_query($query);
	$data = mysql_fetch_array($result);
	$amountin += $data[0];
	$fees += $data[1];
